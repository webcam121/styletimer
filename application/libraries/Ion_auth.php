...
class Ion_auth
{
	...
	{
		 $profile = $this->db->select('*')->where('forgotten_password_code', $code)->limit(1)->order_by('id', 'desc')->get('st_users')->row();
	...
	public function register($identity, $password, $email, $additional_data = [], $group_ids = [])
	{
		...
		else
		{
			...
			{    
                ...
                //$mail = phpmail($email, $this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_activation_subject'),$message);
	...
	{ 
		...
		$identity = $this->config->item('identity', 'ion_auth');
		$session_data = [
		    $identity,'identity','email','st_userid','id','old_last_login','last_check','access','status','sty_fname','sty_profile','user_id'
		];
