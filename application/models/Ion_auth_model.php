...
class Ion_auth_model extends CI_Model
{
	...
	public function activate($id, $code = FALSE)
	{
		...
		if ($code !== FALSE) {
			...
			$whr=array('id' =>$id, 'activation_code' => $code);
	...
	public function change_password($identity, $old, $new)
	{
		...
		$this->set_error('password_not_match');
		return FALSE;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->db->update($this->tables['users'], $update, array($this->identity_column => $identity,'status !='=>'deleted'));
	...
	public function forgotten_password_old($identity)
	{
		if (empty($identity))
		{
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		$data = [
			...
			//'status' => ($manual_activation === FALSE ? 'active' : 'inactive')
		];
	 ...
	 * @author    Mathew
	 */
	public function login($identity, $password, $remember=FALSE,$access="")
	{
		...
		$this->trigger_events('extra_where');
                    ...
                    if($access!="admin")
                     {
						$where=array('access!='=>'admin','status !='=>'deleted'); 
					 }else{
						$where=array('access'=>'admin','status !='=>'deleted');  
						 }         
			      $query = $this->db->select($this->identity_column . ', email,business_name,id, password, status, last_login, access, first_name, profile_pic,profile_status,notification_sound_setting')
						  ->where($this->identity_column, $identity)
						  ->where($where)
						  ->limit(1)
						  ->order_by('id', 'desc')
           ...
           // echo $login." | ".$user->access; die;
			...
			if ($this->verify_password($password, $user->password, $identity))
			{
				...
				$this->set_session($user);
				 ...
				 //print_r($this->session->userdata()); die(" eeee"); 
	 ...
	 * @author Ben Edmunds
	 */
	public function get_user_by_code($code = NULL)
	{
		$this->trigger_events('user');

		$query = $this->db->select('*')->where($this->tables['users'].'.login_code', $code)->get($this->tables['users']);
        $user  = $query->row() ;
        return $user;
	}
	public function user_login_by_code($code = NULL)
	{
		$this->trigger_events('user');
        ...
        $user  = $query->row();
         	...
         	if($user){	
					...
					$this->db->update('users', ['login_code' => ''], ['id' => $user->id]);
	...
	public function set_session($user)
	{
		...
		$session_data = [
		    ...
		    'sound_setting'	   => $user->notification_sound_setting
		];
	...
	public function checkemail($data,$table)
	{
	...
	public function checkemailmodifiy($email)
	{
		$this->db->select('*');
	...
	public function forget_update1($id,$data) {

		$this->db->where('id', $id);
		$ups = $this->db->update('st_users', $data);
		if($ups){
             return true;
		}else{
			return false;
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		$query = $this->db->select('password, email')
		                  ->where('id', $id)
		                  ->limit(1)
		                  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() !== 1)
		{
			...
			if ($this->verify_password($password, $user->password, $user->email))
			{
				return TRUE;
			}
			else
