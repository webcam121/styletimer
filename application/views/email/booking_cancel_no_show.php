    ...
    <?php /* <tr>
      ...
      <tr>
        <td style="padding:0px 30px;">
          <p  style="font-size: 20px;color:#333;font-weight:500;line-height: 30px; "></p>
        </td>
      </tr>      
      <tr>
        <td style="padding:0px 30px;">
          <p style="font-size: 16px;font-weight:500;margin-top:5px; ">Hi <?php echo $fname.' '.$lname; ?>,</p>
          <p style="font-size: 16px;font-weight:500;line-height: 20px; ">
            leider bist du nicht zu deinem Termin bei <?php echo $salon_name; ?> erschienen. <br/>
            Bitte bedenke, dass ein Salon bei verpassten Terminen keine Chance hat, den Zeitraum neu zu vergeben.<br/>
            Falls du einen Termin nicht wahrnehmen kannst, storniere ihn bitte so früh wie möglich oder verlege ihn in deinen Buchungsoptionen.<br/>
            Hier die Details der verpassten Buchung:<br/>
          </p>
          <?php if(isset($booking_id)){ ?>
            <p style="font-size:16px;margin-top:0px;margin-bottom:0px;"><?php echo $this->lang->line('Booking_Id'); ?> : <a style="color:#00b3bf;" href="<?php echo base_url('home/booking_redirect/').url_encode($booking_id); ?>">
              <?php if(!empty($book_id)) { echo $book_id;} 
                    else { echo $booking_id; } ?></a></p> 
            <?php } ?>
          <p style="font-size:16px;margin-top:0px;margin-bottom:0px;">Service : <?php echo $service_name; ?></p>
          <p style="font-size:16px;margin-top:0px;margin-bottom:0px;"><?php echo $this->lang->line('Date'); ?> : <?php echo $booking_date; ?></p>
          <p style="font-size:16px;margin-top:0px;margin-bottom:0px;"><?php echo $this->lang->line('Time'); ?> : <?php echo $booking_time; ?> Uhr</p>
          <p style="font-size:16px;margin-top:0px;margin-bottom:0px;">Dauer : <?php echo $duration; ?> Min.</p>
          <?php //echo base_url('booking/detail/').url_encode($booking_id); ?>
        </td>
      </tr>
      <tr>
        <td style="padding:0px 30px;">
