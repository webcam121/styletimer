...
table{
  ...
  <?php if(!empty($booking_detail)){ 
        $time = new DateTime($main[0]->booking_time);
         ...
         $otime = new DateTime($old_date);
         $odate = $otime->format('d.m.Y');
         $otime = $otime->format('H:i');
    ?>
      ...
      <tr style="border-bottom:1px solid #e8e8e8;">
        <td style="padding:0px 30px;">
          <p style="font-size: 16px;font-weight:500;margin-top:5px; ">
            Hi <?php echo $main[0]->salon_name; ?>,
          </p>
          <p style="font-size: 16px;font-weight:500;line-height: 20px; ">
            <?php echo $booking_detail[0]->first_name; if(!empty($booking_detail[0]->last_name)) echo " ".$booking_detail[0]->last_name; ?> hat eine Buchung in deinem Salon verschoben.<br/>
          </p>
        </td>
      </tr>
      <tr>
        <td style="padding: 0px 30px;">
          <p style="font-size: 16px;margin-top:0px;margin-bottom:0px;"><span><?php echo $this->lang->line('Booking_Id'); ?> : </span> <a style="color:#00b3bf;" href="<?php echo base_url('home/booking_redirect/').url_encode($main[0]->id); ?>"><?php if(!empty($main[0]->book_id)) echo $main[0]->book_id; else echo $main[0]->id; ?></a> </p>
          <p style="font-size: 16px;margin-top:0px;margin-bottom:0px;"><span>Service : </span> <?php echo $booking_detail[0]->service_name;?> </p>
          <p style="font-size: 16px;margin-top:0px;margin-bottom:0px;"><span><?php echo $this->lang->line('Date'); ?> : </span> <?php echo date('d.m.Y', strtotime($odate)); ?></p>
          <p style="font-size: 16px;margin-top:0px;margin-bottom:0px;"><span><?php echo $this->lang->line('Time'); ?> : </span> <?php echo $otime; ?> Uhr</p>
          <p style="font-size: 16px;margin-top:0px;margin-bottom:0px;"><span>Neues <?php echo $this->lang->line('Date'); ?> : </span> <?php echo date('d.m.Y', strtotime($date)); ?></p>
          <p style="font-size: 16px;margin-top:0px;margin-bottom:0px;"><span>Neue <?php echo $this->lang->line('Time'); ?> : </span> <?php echo $time; ?> Uhr</p>
          <p style="font-size: 16px;margin-top:0px;margin-bottom:0px;"><span>Dauer : </span> <?php if(!empty($main[0]->total_time)) echo $main[0]->total_time; else '-'; ?> Min.</a> </p>
          <?php //echo base_url('booking/detail/').url_encode($main[0]->id); ?>
        </td>
      </tr>
      <tr>
        <td style="padding:0px 30px;">
          <p style="font-size: 16px;font-weight:500;margin-top:10px; "><?php echo $this->lang->line('Have_nice_day'); ?></p>
