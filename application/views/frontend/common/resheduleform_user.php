							   ...
							   for($i=0;$i<=$maxdays;$i += 7)
                                  {
								   ...
								   <div class="slider-wick display-b  week<?php echo date('W',strtotime($monthCurrent)); ?>">								     
									 <p class="color333 font-size-16 fontfamily-medium mt-0 mb-20"><?php echo date('M Y',strtotime($setMonthDate)); ?></p>
											  ...
											  // echo $row;
														 ...
														 $day = strtolower(date("l",strtotime($currentplus))); ?>
															  ...
															  <div class="same-width color999 display-b">							
																<span class="d-block <?php if(in_array($day,$days)) echo 'unslect'; ?>">
																	<?php 
																 ...
																 <div class="radiobox-image <?php if(in_array($day,$days)) echo 'unslect'; ?>">
																  <label class="color666 pl-0">
																	<input type="radio" name="chg_date" data-month="<?php echo date('M Y',strtotime($currentplus)); ?>"; data-slide="<?php echo $slide; ?>" class="chg_date_u <?php if($currentplus==$checkeddate) echo "selectedDate"; ?>" value="<?php echo $currentplus; ?>" <?php if($currentplus==$checkeddate) echo "checked='checked'"; ?>>
																	   <span class="date-chack">
																		  <span class="date-chacked"><?php echo date('d',strtotime($currentplus)); ?></span>
																	   </span>
																  </label>                                
																</div>
															  </div>
...
<!--------------------- Time Slot Start ----------------------------------------------------------------------------------------------------------------------------->
	 ...
	 <input type="hidden" id="resmid" name="resmid" value="<?php echo $merchant_id; ?>">
	  
	  <div class="text-center">
		<button type="button" id="resheduleSubByuser" class="btn widthfit"><?php echo $this->lang->line('Submit'); ?></button>
	</div>
	</div>
