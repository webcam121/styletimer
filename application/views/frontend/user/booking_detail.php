...
<section class="pt-120 clear user_booking_section1">
	<div class="container">
		<div class="row">
	...
	<div class="modal-dialog modal-md modal-dialog-centered" role="document">
		<div class="modal-content text-center">
			...
			<div class="modal-body pt-30 mb-30 pl-40 pr-40">
				<img src="<?php echo base_url('assets/frontend/images/thankyou_top_img2.svg'); ?>" class="mb-3" style="width:150px;">
				<h4 class="color333 fontfamily-medium font-size-24">Services Booked Successfully</h4>
				<p class="mb-30 color666 font-size-16 fontfamily-regular">Your service booking has been done successfully. We have sent a confirmation mail to your registered Email Id. </p>
				<a href="<?php echo base_url('user/all_bookings'); ?>" class=" btn btn-large widthfit">ok</a>
			</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	jQuery(document).ready(function($) {
		...
		$(document).on('change', '.selectdate,.selectEmployee, .slectTime', function() {

			var formData = $('#selectFilters').serialize();
			formData = formData + "&is_ajax=true";
			var merchId = $('input[name="merch_id"]').val();

			loading();

			$.ajax({
				url: `<?php echo base_url('/booking/booking_detail');?>/${merchId}`,
				data: formData,
				method: 'get',
				success: function(response) {
					$('.user_booking_section1').html(response).fadeIn()
					unloading();
				}
			});
		...
		function setEmployeeList(employeeList) {
			...
			var empListHtml = '';
			var selectedEmployee = $('input[name="employee_select"]').val();
			...
			employeeList.forEach(employee => {
				empListHtml += `
				<li class = "radiobox-image">
				<input type="radio" id="id_employee${employee.id}" name="employee_select" class="selectEmployee" value=""
			      data-text="${employee.first_name}" data-src="${employee.profile_pic !== '' ? baseUrl+'/assets/uploads/employee/'+employee.id+'/'+employee.profile_pic: baseUrl+'/assets/frontend/images/user-icon-gret.svg'}"	checked="${selectedEmployee == employee.first_name ? 'checked':''}">

				<label for="id_employee${employee.id}">
				<img class = "employee-round-icon" src = "${employee.profile_pic !== '' ? baseUrl+'/assets/uploads/employee/'+employee.id+'/'+employee.profile_pic: baseUrl+'/assets/frontend/images/user-icon-gret.svg'}" > ${employee.first_name} </label> 
				</li>`
			});

			$('#employeeList').html(empListHtml);
		}



		//~ $('#dropdownTime').on('shown.bs.dropdown', function () {
		//~ var ele = $(this).find("li>input:class('selected_time')");
